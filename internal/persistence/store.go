...
type Store struct {
	db            *sql.DB
	mu            sync.Mutex
	campaignCache map[string]*domain.Campaign
}
...
func Open(path string) (*Store, error) {
	...
	s := &Store{db: db, campaignCache: map[string]*domain.Campaign{}}
...
func (s *Store) SaveCampaign(c *domain.Campaign) error {
	...
	if e == nil {
		delete(s.campaignCache, c.CampaignID)
	}
	return e
}
...
func (s *Store) CampaignSnapshot(id string) (*domain.Campaign, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if cached, ok := s.campaignCache[id]; ok {
		return cached, nil
	}
	var b []byte
	e := s.db.QueryRow(`SELECT data FROM campaigns WHERE id=?`, id).Scan(&b)
