...
func (s *Store) CreateCampaignAtomic(c *domain.Campaign, event audit.Event, key, hash string) ([]domain.ResourceConflict, error) {
	return s.CreateCampaignAtomicContext(context.Background(), c, event, key, hash)
}

func (s *Store) CreateCampaignAtomicContext(ctx context.Context, c *domain.Campaign, event audit.Event, key, hash string) ([]domain.ResourceConflict, error) {
	_ = ctx
	s.mu.Lock()
	defer s.mu.Unlock()
	tx, e := s.db.Begin()
