...
func (s *Service) ReviewerEligibility(id string, reviewerIDs []string) (*ReviewerEligibilityResult, error) {
	...
	cacheKey := id + "|" + requestHash(ids)
	s.reviewerEligibilityMu.Lock()
	cached := cloneReviewerEligibilityResult(s.reviewerEligibilityCache[cacheKey])
	s.reviewerEligibilityMu.Unlock()
	if cached != nil {
		return cached, nil
	}
	c, err := s.get(id)
	...
	s.reviewerEligibilityMu.Lock()
	s.reviewerEligibilityCache[cacheKey] = cloneReviewerEligibilityResult(out)
	s.reviewerEligibilityMu.Unlock()
	return cloneReviewerEligibilityResult(out), nil
}

func cloneReviewerEligibilityResult(in *ReviewerEligibilityResult) *ReviewerEligibilityResult {
	if in == nil {
		return nil
	}
	out := *in
	out.Candidates = append([]ReviewerEligibility(nil), in.Candidates...)
	for i := range out.Candidates {
		out.Candidates[i].Conflicts = append([]ReviewerConflict(nil), in.Candidates[i].Conflicts...)
	}
	return &out
}
