...
func (s *Service) CompareArtifacts(sourceID, targetID string) (*ArtifactComparison, error) {
	...
	cacheKey := sourceID + "\x00" + targetID
	s.artifactComparisonMu.Lock()
	cached := s.artifactComparisonResults[cacheKey]
	s.artifactComparisonMu.Unlock()
	if cached != nil {
		return cached, nil
	}
	...
	s.artifactComparisonMu.Lock()
	if s.artifactComparisonResults == nil {
		s.artifactComparisonResults = map[string]*ArtifactComparison{}
	}
	s.artifactComparisonResults[cacheKey] = out
	s.artifactComparisonMu.Unlock()
	return out, nil
}
