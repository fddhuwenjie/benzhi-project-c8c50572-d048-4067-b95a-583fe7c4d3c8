...
func (s *Service) reviewSnapshotIfPresent(id string, revision int64) (*domain.ReviewSnapshot, error) {
	snapshot, err := s.Store.ReviewSnapshot(id, revision)
	if err != nil {
		return nil, nil
	}
	return snapshot, nil
...
func (s *Service) CreateReviewSnapshot(id, reviewer string) (*ReviewSnapshotResult, error) {
	...
	if old, e := s.reviewSnapshotIfPresent(id, c.Revision); e == nil && old != nil {
