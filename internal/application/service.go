...
func New(store *persistence.Store) *Service { return &Service{Store: store, Audit: &audit.Log{}} }
...
func canonicalizeMeasurementBatch(batch []domain.MeasurementRound) {
	sort.SliceStable(batch, func(i, j int) bool {
		if batch[i].Sequence == batch[j].Sequence {
			return batch[i].RoundID < batch[j].RoundID
		}
		return batch[i].Sequence < batch[j].Sequence
	})
	for i := range batch {
		sort.SliceStable(batch[i].Samples, func(a, b int) bool {
			if batch[i].Samples[a].DeviceID == batch[i].Samples[b].DeviceID {
				return batch[i].Samples[a].SampledAt.Before(batch[i].Samples[b].SampledAt)
			}
			return batch[i].Samples[a].DeviceID < batch[i].Samples[b].DeviceID
		})
		for key, value := range batch[i].Environment {
			batch[i].Environment[key] = strings.TrimSpace(value)
		}
	}
}
