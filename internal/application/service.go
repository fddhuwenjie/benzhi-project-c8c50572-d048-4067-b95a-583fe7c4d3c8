...
type Service struct {
	...
	verificationCache map[string]map[string]any
}

func New(store *persistence.Store) *Service {
	return &Service{Store: store, Audit: &audit.Log{}, verificationCache: map[string]map[string]any{}}
}
...
func (s *Service) VerifySection(id, section string) map[string]any {
	cacheKey := id + "\x00" + section
	s.verificationMu.RLock()
	if cached := s.verificationCache[cacheKey]; cached != nil {
		out := cloneVerification(cached)
	...
	s.verificationCache[cacheKey] = cloneVerification(out)
	s.verificationMu.Unlock()
	return out
}

func cloneVerification(source map[string]any) map[string]any {
