...
type Service struct {
	...
	Audit                   *audit.Log
	measurementSummaryMu    sync.Mutex
	measurementSummaryCache map[measurementSummaryCacheKey]domain.MeasurementSummary
}

type measurementSummaryCacheKey struct {
	campaignID string
	revision   int64
	deviceID   string
	purpose    string
...
func New(store *persistence.Store) *Service {
	return &Service{Store: store, Audit: &audit.Log{}, measurementSummaryCache: map[measurementSummaryCacheKey]domain.MeasurementSummary{}}
}
