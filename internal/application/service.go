...
type Service struct {
	...
	Audit                       *audit.Log
	environmentCorrelationMu    sync.RWMutex
	environmentCorrelationCache map[string]domain.EnvironmentCorrelation
}

func New(store *persistence.Store) *Service {
	return &Service{Store: store, Audit: &audit.Log{}, environmentCorrelationCache: map[string]domain.EnvironmentCorrelation{}}
}
