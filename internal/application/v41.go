...
func (s *Service) ResourceAvailability(station string, start, end time.Time, devices []string) (domain.ResourceAvailability, error) {
	key := requestHash(struct {
		Station string
		Start   time.Time
		End     time.Time
		Devices []string
	}{station, start.UTC(), end.UTC(), devices})
	s.availabilityMu.Lock()
	if raw, ok := s.resourceAvailabilityCache[key]; ok {
		var cached domain.ResourceAvailability
	...
	result, err := domain.EvaluateResourceAvailability(station, start, end, devices, items, time.Now().UTC())
	if err != nil {
		return domain.ResourceAvailability{}, err
	}
	...
	s.availabilityMu.Lock()
	s.resourceAvailabilityCache[key] = raw
	s.availabilityMu.Unlock()
	return result, nil
}
...
func (s *Service) RemediationDependencies(id string) (*domain.DependencyProjection, error) {
	...
	edges, err := s.Store.RemediationDependencies(id)
	if err != nil {
		return nil, err
	}
