...
func (s *Service) MeasurementSummary(id, deviceID, purpose string) (domain.MeasurementSummary, error) {
	...
	key := measurementSummaryCacheKey{campaignID: id, revision: c.Revision, deviceID: deviceID, purpose: purpose}
	s.measurementSummaryMu.Lock()
	if cached, ok := s.measurementSummaryCache[key]; ok {
		s.measurementSummaryMu.Unlock()
		return cached, nil
	}
	s.measurementSummaryMu.Unlock()
	rounds, e := s.Store.Rounds(id)
	...
	result := domain.BuildMeasurementSummary(c, effectiveWithoutExclusions(rounds, voids, exclusions), deviceID, purpose)
	s.measurementSummaryMu.Lock()
	s.measurementSummaryCache[key] = result
	s.measurementSummaryMu.Unlock()
	return result, nil
}
