...
func (s *Service) CorrectReference(id, requestID string, in CorrectionInput) (*ReferenceResult, error) {
	...
	e = s.Store.Commit(persistence.Mutation{Campaign: c, References: []domain.ReferenceEvidence{n}, UpdateReferences: []domain.ReferenceEvidence{refs[idx]}, Event: &ev, IdemKey: key, IdemHash: hash, Response: result})
	return result, e
}
...
type evaluationHistoryCacheKey struct {
	CampaignID string
	Revision   int64
	Offset     int
	Limit      int
...
func (s *Service) EvaluationHistory(id string, offset, limit int, from, to int64) (*EvaluationHistory, error) {
	...
	key := evaluationHistoryCacheKey{CampaignID: id, Revision: campaign.Revision, Offset: offset, Limit: limit, From: from, To: to}
	s.evaluationHistoryMu.Lock()
	cached := s.evaluationHistoryResults[key]
	s.evaluationHistoryMu.Unlock()
	if cached != nil {
		return cached, nil
	}
	...
	s.evaluationHistoryMu.Lock()
	if s.evaluationHistoryResults == nil {
		s.evaluationHistoryResults = map[evaluationHistoryCacheKey]*EvaluationHistory{}
	}
	s.evaluationHistoryResults[key] = out
	s.evaluationHistoryMu.Unlock()
	return out, nil
}
